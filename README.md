...
<p align="center">
  <img width="100%" height="100%" src="https://miro.medium.com/v2/resize:fit:828/format:webp/1*age3Dgxl8sz6LZgDIwOSNQ.png">
</p>

## About
This project was completed as part of the 42 core curriculum, developed in collaboration with ____ and . The goal was to build an HTTP web server from scratch, compatible with C++98. The web server can handle HTTP GET, HEAD, POST, PUT, and DELETE requests, serving static files from a specified root directory or dynamic content using CGI. It supports multiple concurrent client connections using select().
...
__HTTP Basics___
HTTP (Hypertext Transfer Protocol) is the protocol used for transferring information over the internet, forming the foundation of the World Wide Web. A web server listens for and responds to HTTP requests from clients (like web browsers). An HTTP transaction involves a request from the client and a response from the server.
...
An HTTP request includes a request line, headers, and an optional message body. Example:
...
- Message Body: Optional, typically absent in GET requests.

__HTTP Response__

An HTTP response includes a status line, headers, and an optional message body. Example:
